import { ADDRESS_FORM_FIELDS } from '@/features/address/domain/address-constants'
import { t } from '@/infrastructure/i18n'
import { TextField, type TextFieldProps } from '@/presentation/components/forms/text-field'

export const AddressStreetField: React.FC<Partial<TextFieldProps>> = ({
  label = t('address.fields.street.label'),
  name = ADDRESS_FORM_FIELDS.STREET,
  placeholder = t('address.fields.street.placeholder'),
  ...addressStreetFieldRestProps
}) => (
  <TextField {...addressStreetFieldRestProps} label={label} name={name} placeholder={placeholder} />
)
